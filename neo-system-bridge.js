(function () {
  ...
  function ready(){scan(document);new MutationObserver(records=>records.forEach(r=>r.addedNodes.forEach(node=>{scan(node);if(node.matches?.('iframe'))node.addEventListener('load',()=>sendPreferences(node.contentWindow));node.querySelectorAll?.('iframe').forEach(frame=>frame.addEventListener('load',()=>sendPreferences(frame.contentWindow)));}))).observe(document.documentElement,{childList:true,subtree:true});document.querySelectorAll('iframe').forEach(frame=>frame.addEventListener('load',()=>sendPreferences(frame.contentWindow)));syncFrames();}
